...
\author{
Maximo Q.~Menchaca\thanks{ Use footnote for providing further information
...
\subsection{Results}
The output probabilities are plagued by underflows, where the probability of being in a particular hidden model state after many steps is too small to be tracked. A simple fix is to compute log probabilites instead of raw probabilities. Currently, underflow errors are treated as non-predictive, but predictive states show a .525 root-mean-square-error.

\section{Future Work}
We hope to implement the Baum-Welch algorithm to train our HMM transition and emission probabilities instead of our biased guesses.
